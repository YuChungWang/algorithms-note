// You are given an array of integers numbers and two integers left and right. You task is to calculate a boolean array result,
...
// * For numbers[0] = 8, we need to find a value of x such that 1 * x = 8, but the only value that would work is x = 8
// which doesn't satisfy the boundaries 1 ≤ x ≤ 3, so result[0] = false.
...
const solution = (numbers, left, right) => {
  let result = Array(numbers.length);
  
  for (let i = 0; i < numbers.length; i++) {
    const x = numbers[i] / (i + 1);
    result[i] = numbers[i] % (i + 1) === 0 && x >= left && right >= x;
  }
  return result;
};
