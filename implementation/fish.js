...
// and eats it, then it meets fish number 3 and eats it too. Finally, it meets fish number 4 and is eaten by it. The remaining
...
const solution = (A, B) => {
  ...
  for (let i = 0; i < A.length; i++) {
    if (B[i] === 1) {
      ...
      while (downstreamStack.length) {
        if (A[i] > A[downstreamStack[downstreamStack.length - 1]]) {
          downstreamStack.pop();
        } else {
          break;
        }
  ...
  return count + downstreamStack.length;
}
