...
// grid as 1.
// A board that redirects the ball to the left spans the top-right corner to the bottom-left corner and is represented in the
// grid as -1.
...
// all from the ith column at the top, or -1 if the ball gets stuck in the box.
...
const findBall = (grid) => {
  const posArr = [...Array(grid[0].length)].map((_, idx) => idx);
  
  for (let row = 0; row < grid.length; row++) {
    for (let posIdx = 0; posIdx < posArr.length; posIdx++) {
      if (posArr[posIdx] === -1) {
        continue;
      }
      
      if (grid[row][posArr[posIdx]] === 1) {
        if ([-1, undefined].includes(grid[row][posArr[posIdx] + 1])) {
          posArr[posIdx] = -1;
        } else {
          posArr[posIdx] += 1;
        }
