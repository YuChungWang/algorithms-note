...
const pacificAtlantic = (heights) => {
  ...
  const dfs = (row, col, type) => {
    ...
    dir.forEach(([offsetRow, offsetCol]) => {
      ...
      if (
        (type === 'Pacific' && !pMap[`${newRow}-${newCol}`] && heights[newRow][newCol] >= heights[row][col]) ||
        (type === 'Atlantic' && !aMap[`${newRow}-${newCol}`] && heights[newRow][newCol] >= heights[row][col])
      ) {
        dfs(newRow, newCol, type);
      }
    });
  };
