...
// answers, return any of them. It is guaranteed that there will be at least one valid solution for the given
// input.
...
const groupThePeople = (groupSizes) => {
  ...
  for (let i = 0; i < groupSizes.length; i++) {
    sizeMap.set(groupSizes[i], [...(sizeMap.get(groupSizes[i]) || []), i]);
  }
  
  let currGroup = [];
  for (const [groupSize, personArr] of sizeMap.entries()) {
    for (let person of personArr) {
      currGroup.push(person);
